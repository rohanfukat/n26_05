...
genai.configure(api_key="AIzaSyBhdtc0555C5C0Weuad2a7ap7RL9lFHB5o")
...
DUPLICATE_WARNING_THRESHOLD = 0.65  # 65% similarity = potential duplicate
...
EMERGENCY_KEYWORDS = ["blast", "explosion", "fire", "gas leak", "accident", "collapse"]

GRIEVANCE_KEYWORDS = [
    "complaint", "issue", "problem", "broken", "not working",
    "leak", "garbage", "waste", "road", "pothole",
    "traffic", "water", "electricity", "sewage"
]

# ================= HELPERS =================

def is_grievance(text: str):
    return any(k in text.lower() for k in GRIEVANCE_KEYWORDS)
...
def extract_location(text: str):
    ...
    # Comprehensive list of Mumbai areas from data.json
    locations = {
        # South Mumbai
        "colaba", "fort", "worli", "parel", "lower parel", "byculla",
        # Central Mumbai  
        "dadar", "dadar west", "dadar east", "matunga", "matunga west",
        "mahim", "bandra", "bandra west", "bandra east", "kala ghoda",
        # West Mumbai
    ...
    # Check for area names (longer matches first to avoid "west" matching before "west mumbai")
...
def check_duplicate(new_embedding, category, location, threshold=DUPLICATE_THRESHOLD):
    ...
    Args:
        new_embedding: Vector representation of new complaint
        category: Complaint category
        location: Geographic location
        threshold: Similarity threshold for duplicate (0-1)
    
    Returns:
        (is_duplicate, confidence_score, matched_id)
    """
    best_score = 0
    best_id = None
    ...
    for c in complaint_memory:
        ...
        # Only compare same location (if both available)
    ...
    # Return duplicate status based on threshold
    is_duplicate = best_score >= threshold
    return is_duplicate, best_score, best_id
...
async def classify_text_only(cleaned_text: str, raw_text: str):
    ...
    if not is_grievance(raw_text):
        return {
            "category": "non-grievance",
            "processed_by": "filter"
        }
    ...
    try:
        ...
        # Use keyword matching for better accuracy
        department = "General Administration (BMC)"
        
        # Check text first (higher priority)
        text_lower = raw_text.lower()
        keywords_sorted = sorted(DEPARTMENT_MAP.keys(), key=len, reverse=True)
        for keyword in keywords_sorted:
            if keyword in text_lower:
                department = DEPARTMENT_MAP[keyword]
                break
        
        # Check category if text matching didn't work
        if department == "General Administration (BMC)":
...
def compare_analyses(text_result, image_result):
    ...
    if set(text_result.get("tags", [])) & set(image_result.get("tags", [])):
        score += 10
...
async def batch_classify_text(complaints):
    results = []
    for c in complaints:
        res = await classify_text_only(c["text"], c["text"])
        results.append(res)
    return results

async def batch_analyze_and_compare_images(items):
    results = []
    for i in items:
        res = await analyze_and_compare(i["text"], i["text"], i["image_bytes"])
        results.append(res)
    return results
