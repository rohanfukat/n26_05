...
export default function NeighborhoodComplaints() {
  ...
  return (
    <PageLayout>
      ...
      {/* ── Learn More Modal (portalled to body to avoid z-index clipping) ── */}
      {createPortal(
        <AnimatePresence>
          {selectedComplaint && (
            <motion.div
            ...
            >
              <motion.div
                ...
                className="w-full max-w-lg max-h-[85vh] flex flex-col overflow-hidden rounded-xl bg-zinc-900 border border-zinc-700/60 shadow-[0_24px_80px_rgba(0,0,0,0.8)]"
              >
                {/* Image */}
                <div className="flex-shrink-0">
                  {(selectedComplaint.before_photo || selectedComplaint.imageUrl) ? (
                    ...
                    <div className="h-32 bg-zinc-800/80 flex items-center justify-center text-zinc-600">
                ...
                <div className="flex-1 overflow-y-auto">
                  ...
                  <div className="flex items-start justify-between p-5 border-b border-zinc-800">
                    <div className="min-w-0">
                      <p className="text-[10px] uppercase tracking-widest text-zinc-500 mb-1">{selectedComplaint.category}</p>
                      <h2 className="text-base font-bold text-white leading-tight">{selectedComplaint.issue || selectedComplaint.title}</h2>
                    </div>
                    <button
                      onClick={() => setSelectedComplaint(null)}
                      className="text-zinc-500 hover:text-white ml-4 flex-shrink-0 p-1 rounded-lg hover:bg-zinc-800 transition-colors"
                    >
                      <X className="h-5 w-5" />
                    </button>
                  </div>
                  ...
                  <div className="p-5 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        ...
                        <div key={label} className="bg-zinc-800/60 border border-zinc-700/50 p-3 rounded-lg">
                    ...
                    <button
                      ...
                      className={`w-full flex items-center justify-center gap-2 py-2.5 border rounded-lg transition-all duration-150 text-sm font-medium ${hasUpvoted(selectedComplaint)
                        ? 'bg-blue-600/20 border-blue-500/50 text-blue-400'
